...
import { TENANT_MESSAGES_TOUR_STEPS as TOUR_STEPS } from "@/lib/product-tour";
...
export function MessagesTour() {
    ...
    useEffect(() => {
        ...
        const hasSeenTour = localStorage.getItem("ireside_messages_tour_completed");
        if (!hasSeenTour) {
    ...
    const completeTour = () => {
        setIsVisible(false);
        localStorage.setItem("ireside_messages_tour_completed", "true");
    };
