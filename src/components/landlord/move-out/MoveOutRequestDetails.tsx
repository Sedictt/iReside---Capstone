        ...
        <div className="flex items-center gap-5">
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-3">
              <h1 className="text-3xl font-bold tracking-tight text-foreground">Move-Out Request</h1>
        ...
        {request.status === "pending" && (
          <div className="flex items-center gap-3">
            <button
              ...
              className="flex h-11 items-center gap-2 rounded-2xl border border-red-500/20 bg-red-500/5 px-6 text-[10px] font-bold uppercase tracking-widest text-red-500 transition-all hover:bg-red-500/10 active:scale-95 disabled:opacity-50"
            >
            ...
            <button
              ...
              disabled={!!actionLoading}
              className="flex h-11 items-center gap-2 rounded-2xl bg-primary px-8 text-[10px] font-bold uppercase tracking-widest text-primary-foreground shadow-lg shadow-primary/20 transition-all hover:bg-primary/90 active:scale-95 disabled:opacity-50"
            >
          ...
          <div className="flex flex-col gap-8 lg:col-span-8">
            ...
            <div className="grid gap-6 md:grid-cols-2">
              ...
              <div className="col-span-full overflow-hidden rounded-3xl border border-border bg-card shadow-sm">
                <div className="grid divide-x divide-border md:grid-cols-2">
                  <div className="flex items-center gap-4 p-6">
                    ...
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Requested Move-Out</p>
                      <p className="text-lg font-bold text-foreground">{formatDate(request.requested_date)}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-4 p-6 bg-muted/10">
                    ...
                    <div>
                      <p className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Current Lease Ends</p>
                      <p className="text-lg font-bold text-foreground">{formatDate(request.lease.end_date)}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-4 rounded-3xl border border-border bg-card p-6 shadow-sm">
                <h3 className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Tenant Profile</h3>
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-base font-bold text-foreground truncate">{request.lease.tenant.full_name}</p>
                    <p className="text-xs font-medium text-muted-foreground truncate">{request.lease.tenant.email}</p>
                  </div>
                </div>
                <div className="flex flex-wrap gap-2 pt-2">
                  <a href={`mailto:${request.lease.tenant.email}`} className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-border px-3 py-2 text-[10px] font-bold uppercase tracking-widest text-muted-foreground transition-all hover:bg-muted">
                    <Mail className="size-3.5" /> Email
                  </a>
                  {request.lease.tenant.phone && (
                    <div className="flex flex-1 items-center justify-center gap-2 rounded-xl border border-border px-3 py-2 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">
              ...
              {/* Property Card */}
              <div className="flex flex-col gap-4 rounded-3xl border border-border bg-card p-6 shadow-sm">
                <h3 className="text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Property & Unit</h3>
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-base font-bold text-foreground truncate">{request.lease.unit.name}</p>
              ...
              >
                <div className="relative z-10 flex flex-col gap-6 md:flex-row md:items-center">
                  <div className="flex-1 space-y-1">
                    <h3 className="text-xl font-bold uppercase tracking-tight">Pending Inspection</h3>
                  ...
                  <button
                    ...
                    className="flex h-12 items-center gap-2 rounded-2xl bg-white px-8 text-[10px] font-bold uppercase tracking-widest text-primary transition-all hover:bg-white/90 active:scale-95"
                  >
                    Start Inspection
            ...
            {request.status === "approved" && request.inspection_date && (
              ...
              >
                <div className="relative z-10 flex flex-col gap-6 md:flex-row md:items-center">
                  <div className="flex-1 space-y-1">
                    <h3 className="text-xl font-bold uppercase tracking-tight text-white">Inspection Completed</h3>
                    <p className="text-sm font-medium text-white/80 max-w-lg">
                      Refund amount: <span className="text-white font-bold">₱{(request.deposit_refund_amount || 0).toLocaleString()}</span>. Finalize the move-out to terminate the lease.
                    </p>
                  </div>
                  <button
                    onClick={handleComplete}
                    disabled={!!actionLoading}
                    className="flex h-12 items-center gap-2 rounded-2xl bg-white px-8 text-[10px] font-bold uppercase tracking-widest text-emerald-600 transition-all hover:bg-white/90 active:scale-95 disabled:opacity-50"
                  >
                    {actionLoading === "approve" ? (
                      <Loader2 className="size-4 animate-spin" />
                    ) : (
          ...
          <div className="flex flex-col gap-6 lg:col-span-4">
            ...
            {(request.status === "completed" || request.checklist_data) && (
              <div className="rounded-3xl border border-border bg-card p-6 shadow-sm">
                <h3 className="mb-6 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Clearance Checklist</h3>
            ...
            {/* Workflow Timeline */}
            <div className="rounded-3xl border border-border bg-card p-6 shadow-sm">
              <h3 className="mb-8 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Workflow Timeline</h3>
            ...
            <div className="rounded-3xl border border-border bg-muted/20 p-6">
              <h3 className="mb-6 text-[10px] font-bold uppercase tracking-widest text-muted-foreground">Lease Financials</h3>
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-bold text-muted-foreground">Monthly Rent</span>
                  <span className="text-sm font-bold text-foreground">₱{(request.lease?.monthly_rent || 0).toLocaleString()}</span>
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-[11px] font-bold text-muted-foreground">Security Deposit</span>
                  <span className="text-sm font-bold text-foreground">₱{(request.lease?.security_deposit || 0).toLocaleString()}</span>
                </div>
                <div className="pt-4 border-t border-border">
                  <button className="flex w-full items-center justify-center gap-2 rounded-xl border border-border bg-card py-2.5 text-[10px] font-bold uppercase tracking-widest text-foreground transition-all hover:bg-muted active:scale-[0.98]">
      ...
      <AnimatePresence>
        {showDenyDialog && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <h3 className="text-2xl font-bold text-foreground tracking-tight">Deny Move-Out</h3>
              ...
              <div className="mt-8 flex gap-3">
                <button
                  onClick={() => setShowDenyDialog(false)}
                  className="flex-1 rounded-2xl border border-border py-3 text-[10px] font-bold uppercase tracking-widest text-foreground transition-all hover:bg-muted active:scale-95"
                >
                  Cancel
                </button>
                <button
                  ...
                  className="flex-1 rounded-2xl bg-red-500 py-3 text-[10px] font-bold uppercase tracking-widest text-white shadow-lg shadow-red-500/20 transition-all hover:bg-red-600 active:scale-95 disabled:opacity-50"
                >
                  {actionLoading === "deny" ? (
