      ...
      <div className={cn(
        ...
        <div className="flex flex-col gap-2">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-2xl font-semibold text-foreground tracking-tight">Move-Out Inspection</h2>
          </div>
        ...
        <div className="grid gap-8 lg:grid-cols-2">
          ...
          <div className="space-y-6">
            <h3 className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">Condition Checklist</h3>
                  ...
                  >
                    <div className="flex flex-col gap-1">
                      <div className="flex items-center gap-2">
                        ...
                        {isSystemChecked && (
                          <div className={cn(
                            "flex items-center gap-1 rounded-full px-2 py-0.5 text-[9px] font-semibold uppercase tracking-tighter",
            ...
            <div className="space-y-3">
              <h3 className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">General Notes</h3>
              <textarea
                value={notes}
            ...
            <div className="space-y-3">
              <h3 className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">Photos</h3>
              <div className="flex h-32 cursor-pointer flex-col items-center justify-center rounded-[2rem] border-2 border-dashed border-border bg-muted/20 text-muted-foreground transition-all hover:border-primary/30 hover:bg-primary/5">
                ...
                <span className="text-[10px] font-semibold uppercase tracking-widest">Upload Photos</span>
          ...
          <div className="flex flex-col gap-8">
            ...
            <div className="rounded-[2.5rem] border border-border bg-card p-8 shadow-sm">
              <div className="mb-8">
                <h3 className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">Outstanding Bills</h3>
              </div>
              
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <div className="space-y-0.5">
                    <p className="text-xs font-semibold text-foreground">Monthly Rent</p>
                ...
                <div className="grid grid-cols-2 gap-4 pt-6 border-t border-border/50">
                  <div className="space-y-1">
                    <p className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">Electricity</p>
                    <p className={cn(
                  ...
                  <div className="space-y-1 border-l border-border/50 pl-4">
                    <p className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">Water</p>
                    <p className={cn(
                      "text-sm font-semibold",
            ...
            <div className="space-y-6">
              <div className="flex items-center justify-between">
                <h3 className="text-[10px] font-semibold uppercase tracking-widest text-muted-foreground">Security Deposit Deductions</h3>
                <button
                  onClick={() => setDeductions([...deductions, { id: Math.random().toString(36).slice(2), description: "", amount: 0 }])}
                  className="flex items-center gap-1.5 rounded-lg border border-primary/20 bg-primary/5 px-3 py-1.5 text-[9px] font-semibold uppercase tracking-widest text-primary transition-all hover:bg-primary/10"
                >
                  ...
                  deductions.map((d) => (
                    ...
                    >
                      ...
                      <div className="relative w-32">
                        <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[10px] font-semibold">₱</span>
                        <input
                          type="number"
                          value={d.amount}
              ...
              <div className="rounded-[2.5rem] border border-border bg-muted/30 p-8 shadow-inner">
                <h3 className="mb-6 text-sm font-semibold uppercase tracking-widest text-muted-foreground">Refund Summary</h3>
                <div className="space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-xs font-bold text-muted-foreground">Security Deposit</span>
                    <span className="text-sm font-semibold text-foreground">₱{originalDeposit.toLocaleString()}</span>
                  </div>
                  <div className="flex items-center justify-between text-red-500">
                    <span className="text-xs font-bold">Total Deductions</span>
                    <span className="text-sm font-semibold">- ₱{totalDeductions.toLocaleString()}</span>
                  </div>
                  <div className="mt-4 flex flex-col gap-1 border-t border-border pt-4">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-semibold uppercase tracking-widest text-foreground">Net Refund</span>
                      <span className="text-2xl font-semibold text-primary">₱{refundAmount.toLocaleString()}</span>
                    </div>
        ...
        <div className="flex justify-end gap-3 border-t border-border pt-8">
          <button
            onClick={onCancel}
            className="rounded-2xl border border-border px-8 py-3 text-[10px] font-semibold uppercase tracking-widest text-foreground transition-all hover:bg-muted"
          >
            Cancel
          </button>
          <button
            ...
            disabled={loading}
            className="flex items-center gap-2 rounded-2xl bg-primary px-10 py-3 text-[10px] font-semibold uppercase tracking-widest text-primary-foreground shadow-lg shadow-primary/20 transition-all hover:bg-primary/90 disabled:opacity-50"
          >
            {loading ? (
            ...
            >
              <div className="flex flex-col gap-6">
                ...
                <div className="space-y-2">
                  <h3 className="text-3xl font-semibold text-foreground tracking-tight">Final Confirmation</h3>
                  <p className="text-sm font-medium text-muted-foreground leading-relaxed">
                    You are about to finalize the move-out inspection. This will determine the final refund of <span className="font-semibold text-primary">₱{refundAmount.toLocaleString()}</span> and notify the tenant.
                  </p>
                </div>
                ...
                <div className="grid gap-3 pt-4 sm:grid-cols-2">
                  <button
                    onClick={() => setShowConfirmSubmit(false)}
                    className="h-14 rounded-2xl border border-border text-[10px] font-semibold uppercase tracking-widest text-foreground transition-all hover:bg-muted"
                  >
                    Go Back
                  </button>
                  <button
                    ...
                    disabled={loading}
                    className="h-14 rounded-2xl bg-primary text-[10px] font-semibold uppercase tracking-widest text-primary-foreground shadow-lg shadow-primary/20 transition-all hover:bg-primary/90 disabled:opacity-50"
                  >
