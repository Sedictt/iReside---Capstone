...
export function Sidebar() {
    ...
    return (
        <aside className="fixed left-0 top-0 z-40 h-screen w-64 border-r border-white/5 bg-[#0f172a] flex flex-col">
            {/* Brand */}
            ...
            <nav className="flex-1 overflow-y-auto px-4 py-8 space-y-8">
                {NAV_ITEMS.map((section) => (
                    <div key={section.category}>
                        <h3 className="mb-4 px-2 text-xs font-bold uppercase tracking-wider text-slate-500">
                            {section.category}
                        </h3>
            ...
            <div className="border-t border-white/5 p-4">
                <div className="flex items-center gap-3 rounded-xl bg-slate-800/30 p-2 hover:bg-slate-800/50 transition-colors cursor-pointer group">
                    <div className="h-10 w-10 overflow-hidden rounded-full bg-emerald-500/20 ring-2 ring-white/10 flex items-center justify-center relative">
                        {/* Placeholder */}
                        <span className="font-bold text-emerald-500 relative z-10">RC</span>
                        <div className="absolute right-0 bottom-0 w-3 h-3 bg-green-500 border-2 border-[#0f172a] rounded-full z-20"></div>
                    </div>
                    <div className="flex-1 overflow-hidden">
                        <p className="truncate text-sm font-semibold text-white group-hover:text-blue-400 transition-colors">
                            Robert Chen
                        </p>
                        <p className="truncate text-xs text-slate-400">Portfolio Manager</p>
                    </div>
