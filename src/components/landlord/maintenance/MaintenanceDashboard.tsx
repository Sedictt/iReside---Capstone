...
export function MaintenanceDashboard() {
 ...
 <Link href="/landlord/maintenance?preview=self_repair_photo" className={cn("text-[10px] sm:text-xs font-black border px-3 py-1.5 rounded-lg transition-colors", previewMode === "self_repair_photo" ? "neumorphic-primary border-primary" : "bg-background text-foreground border-border hover:neumorphic-inset")}>
 Self-Repair Photo
 </Link>
 <Link href="/landlord/maintenance?preview=tenant_repairing" className={cn("text-[10px] sm:text-xs font-black border px-3 py-1.5 rounded-lg transition-colors", previewMode === "tenant_repairing" ? "neumorphic-primary border-primary" : "bg-background text-foreground border-border hover:neumorphic-inset")}>
 Tenant Repairing
 </Link>
 <Link href="/landlord/maintenance?preview=tenant_done" className={cn("text-[10px] sm:text-xs font-black border px-3 py-1.5 rounded-lg transition-colors", previewMode === "tenant_done" ? "neumorphic-primary border-primary" : "bg-background text-foreground border-border hover:neumorphic-inset")}>
 Tenant Done
 </Link>
 <Link href="/landlord/maintenance?preview=third_party_tenant_done" className={cn("text-[10px] sm:text-xs font-black border px-3 py-1.5 rounded-lg transition-colors", previewMode === "third_party_tenant_done" ? "neumorphic-primary border-primary" : "bg-background text-foreground border-border hover:neumorphic-inset")}>
 Third-Party (Tenant Done)
 </Link>
 <Link href="/landlord/maintenance?preview=resolved" className={cn("text-[10px] sm:text-xs font-black border px-3 py-1.5 rounded-lg transition-colors", previewMode === "resolved" ? "neumorphic-primary border-primary" : "bg-background text-foreground border-border hover:neumorphic-inset")}>
 Resolved
 </Link>
 <Link href="/landlord/maintenance" className="text-[10px] sm:text-xs font-black neumorphic-panel px-3 py-1.5 rounded-lg hover:neumorphic-inset transition-colors text-red-500 hover:text-red-600">
 Clear Preview
 </Link>
 </div>
 </div>
 ...
 {/* Main Content Area */}
 <div className="rounded-[2rem] neumorphic-panel flex flex-col ">
 {/* Toolbar */}
 <div className="p-6 border-b border-white/5 neumorphic-inset">
 ...
 className="w-full neumorphic-panel rounded-xl py-2.5 pl-11 pr-4 text-sm font-medium text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-4 focus:ring-primary/10 focus:border-primary/50 transition-all "
 />
 </div>
 </div>

 <div className="flex flex-wrap items-center gap-3 lg:justify-end">
 <div className="flex items-center gap-2 bg-background/50 /50 rounded-xl px-3 py-1.5 ">
 ...
 <div className="rounded-2xl border border-red-500/20 bg-red-50/50 dark:bg-red-500/5 p-6 text-sm text-red-600 dark:text-red-300 ">
 {error}
 </div>
 ) : (
 <div className={cn(
 ...
 <div className="size-20 neumorphic-inset rounded-full flex items-center justify-center mb-4">
...
function MaintenanceCard({ request, onClick }: { request: MaintenanceRequest, onClick?: () => void }) {
 ...
 <div
 onClick={onClick}
 className="group relative flex flex-col neumorphic-panel rounded-3xl overflow-hidden hover:border-primary/30 transition-all duration-300 cursor-pointer h-full"
 >
 {/* Top Image Preview Area */}
 <div className="relative h-48 w-full neumorphic-inset border-b border-white/5 overflow-hidden shrink-0">
 ...
 <div className="absolute top-3 right-3 bg-black/60 backdrop-blur-md px-2 py-1 rounded-lg flex items-center gap-1.5 text-[10px] font-black text-white ">
 ...
 <div className="w-full h-full flex items-center justify-center neumorphic-inset absolute inset-0">
 ...
 {/* Header & Meta */}
 <div className="flex items-center justify-between mb-3 text-[10px] font-black text-muted-foreground uppercase tracking-widest shrink-0">
 ...
 <div className="p-1.5 neumorphic-inset rounded-lg">
 ...
 <span className="text-xs font-black text-foreground truncate max-w-[100px]">{request.tenant}</span>
 </div>
 </div>
 ...
 className="p-2.5 rounded-xl neumorphic-primary hover:bg-primary/90 transition-all active:scale-95"
...
function MaintenanceListRow({ request, onClick }: { request: MaintenanceRequest, onClick?: () => void }) {
 ...
 <div
 onClick={onClick}
 className="group relative flex items-center neumorphic-panel rounded-2xl overflow-hidden hover:border-primary/30 transition-all duration-300 cursor-pointer p-4 gap-6"
 >
 {/* Left Section: Tenant & ID */}
 <div className="flex items-center gap-4 shrink-0 w-[180px]">
 <div
 className="relative size-10 rounded-full flex items-center justify-center text-xs font-black text-white overflow-hidden shrink-0 "
 style={{ backgroundColor: request.tenantAvatarBgColor || '#c4b0ff' }}
 >
 {request.tenantAvatar ? (
...
function StatusBadge({ status, tenantRepairStatus }: { status: Status, tenantRepairStatus?: string }) {
 ...
 "In Progress": "neumorphic-primary border-primary ",
 ...
 customClass = "neumorphic-primary border-primary ";
...
function MaintenanceCardSkeleton({ view }: { view?: string }) {
 if (view === "list") {
 return (
 <div className="relative flex items-center bg-card/60 rounded-2xl p-4 gap-6 h-20 overflow-hidden ">
 ...
 <div className="size-10 neumorphic-inset rounded-full shrink-0" />
 <div className="w-32 h-4 neumorphic-inset rounded" />
 <div className="flex-1 space-y-2">
 <div className="h-4 w-1/2 neumorphic-inset rounded" />
 <div className="h-3 w-1/3 neumorphic-inset rounded" />
 </div>
 <div className="hidden md:flex gap-3">
 <div className="h-7 w-24 neumorphic-inset rounded-lg" />
 <div className="h-7 w-24 neumorphic-inset rounded-lg" />
 </div>
 </div>
 );
 ...
 return (
 <div className="relative flex flex-col neumorphic-panel rounded-3xl overflow-hidden h-[420px]">
 ...
 <div className="h-44 w-full neumorphic-inset animate-pulse" />
 
 <div className="p-4 flex flex-col flex-1 space-y-4">
 <div className="flex justify-between">
 <div className="h-3 w-16 neumorphic-inset rounded animate-pulse" />
 </div>
 
 <div className="space-y-2">
 <div className="h-5 w-full neumorphic-inset rounded-lg animate-pulse" />
 <div className="h-5 w-2/3 neumorphic-inset rounded-lg animate-pulse" />
 </div>

 <div className="flex gap-2">
 <div className="h-6 w-20 neumorphic-inset rounded-md animate-pulse" />
 <div className="h-6 w-16 neumorphic-inset rounded-md animate-pulse" />
 </div>

 <div className="flex-1" />

 <div className="flex items-center justify-between pt-4 border-t border-border">
 <div className="h-4 w-24 neumorphic-inset rounded animate-pulse" />
 <div className="h-9 w-24 neumorphic-inset rounded-xl animate-pulse" />
 </div>
 </div>
 </div>
 );
