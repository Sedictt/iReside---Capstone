...
export function WalkInApplicationModal({
    ...
    const sortUnitsByAvailability = useCallback((unitList: WalkInUnit[]) => {
    ...
    }, [filteredUnits, sortUnitsByAvailability]);
    ...
    const handleSubmit = async (asPending = false) => {
        if (!existingApplication) {
            ...
            if (isUnitOccupied(chosenUnit)) {
                setFormErrors((prev) => ({ ...prev, unit: "Selected unit is currently occupied and unavailable." }));
                setTouchedFields((prev) => ({ ...prev, unit: true }));
                setStep(0);
                setError("Selected unit is currently occupied. Please select an available unit.");
                return;
            }
            if (isUnitOngoing(chosenUnit)) {
                setFormErrors((prev) => ({ ...prev, unit: "Selected unit currently has an ongoing application." }));
                setTouchedFields((prev) => ({ ...prev, unit: true }));
                setStep(0);
                setError("Selected unit currently has an ongoing application under processing. Please select an available unit.");
                return;
            }
        }
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <div className="relative z-10 flex-1">
                                        ...
                                        <p className={cn("text-[9px] uppercase tracking-widest font-black", isActive ? "text-primary opacity-80" : "text-zinc-500 dark:text-neutral-600")}>
                                            {s.sub}
                                        </p>
                                    </div>
                                    {isActive && (
                                                     ...
                                                     <Building className="pointer-events-none absolute left-6 top-1/2 -translate-y-1/2 text-muted-foreground transition-all duration-300 group-focus-within:text-primary" size={20} />
                                                         <select
                                                             ...
                                                             onChange={(e) => {
                                                                 ...
                                                                 } else if (chosen && isUnitOngoing(chosen)) {
                                                                     liveErrors.unit = "Selected unit currently has an ongoing application.";
                                                                 }
...
<GlassInput
                                        ...
                                        <section className="space-y-6">
                                            ...
                                            {formErrors.message && <p className="text-[10px] text-red-400 font-black uppercase tracking-wider ml-1 mt-1">{formErrors.message}</p>}
                                        </section>
                                    </div>
                                )}
                                                ...
                                                {leaseData.signing_mode === "in_person" && (
                                                            ...
                                                            <div className="flex items-center gap-4">
                                                                ...
                                                                {leaseData.tenant_signature && (
                                                                    <div className="flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/30">
                                                                        ...
                                                                        <span className="text-[10px] font-black text-emerald-400 uppercase tracking-wider">Signed</span>
                                                                    </div>
                                                                )}
                                                            </div>
                                                            ...
                                                            <p className="px-1 text-xs text-muted-foreground">Tenant must sign first before landlord can countersign</p>
                                                                ...
                                                                <div className="space-y-4">
                                                                    <div className="p-6 rounded-2xl border border-emerald-500/30 bg-emerald-500/5">
                                                                        <Image 
                                                                            src={leaseData.tenant_signature} 
                                                                            alt="Tenant Signature" 
                                                                            width={300}
                                                                            height={160}
                                                                            unoptimized
                                                                            className="max-h-40 mx-auto w-auto h-auto" 
                                                                        />
                                                                    </div>
                                                                    <button
                                                                        type="button"
                                                                        onClick={() => setLeaseData(prev => ({ ...prev, tenant_signature: null, landlord_signature: null  }))}
                                                                        className="h-12 w-full rounded-xl border border-border bg-background text-sm font-black uppercase tracking-wider text-foreground transition-all hover:bg-muted"
                                                                    >
                                                                        Clear & Re-sign
                                                                    </button>
                                                                </div>
                                                    ...
                                                    <p className="text-center text-sm text-muted-foreground">Waiting for tenant signature first...</p>
                                                                ...
                                                                <div className="space-y-4">
                                                                    <div className="p-6 rounded-2xl border border-emerald-500/30 bg-emerald-500/5">
                                                                        <Image 
                                                                            src={leaseData.landlord_signature} 
                                            ...
                                            allRequirementsMet ? "border-emerald-500/30 shadow-emerald-500/5" : "border-amber-500/30 shadow-amber-500/5"
                                             ...
                                             <div className="p-8 flex flex-col sm:flex-row gap-8 items-center relative z-10">
                                                 ...
                                                 <div className="text-center sm:text-left space-y-2">
                                                     ...
                                                     <p className="max-w-lg text-xs font-black leading-relaxed text-muted-foreground">
                                                         {allRequirementsMet 
