...
type MessageReportAction = 
  | { type: "SET_CATEGORY"; payload: MessageReportCategory }
  | { type: "SET_DETAILS"; payload: string }
  | { type: "SET_EXACT_MESSAGE"; payload: string }
  | { type: "SET_REPORTED_MESSAGE_ID"; payload: string }
  | { type: "SET_SCREENSHOTS"; payload: { files: File[]; previews: string[] } }
  | { type: "REMOVE_SCREENSHOT"; payload: number }
  | { type: "SET_SUBMITTING"; payload: boolean }
  | { type: "SET_ERROR"; payload: string | null }
  | { type: "RESET" };

function messageReportReducer(state: MessageReportState, action: MessageReportAction): MessageReportState {
  switch (action.type) {
    case "SET_CATEGORY":
      return { ...state, category: action.payload };
    case "SET_DETAILS":
      return { ...state, details: action.payload };
    case "SET_EXACT_MESSAGE":
      return { ...state, exactMessage: action.payload };
    case "SET_REPORTED_MESSAGE_ID":
      return { ...state, reportedMessageId: action.payload };
    case "SET_SCREENSHOTS":
      return {
        ...state,
        screenshots: [...state.screenshots, ...action.payload.files],
        screenshotPreviews: [...state.screenshotPreviews, ...action.payload.previews]
      };
    case "REMOVE_SCREENSHOT":
      const newScreenshots = state.screenshots.filter((_, i) => i !== action.payload);
      const newPreviews = state.screenshotPreviews.filter((_, i) => i !== action.payload);
      return {
        ...state,
...
export function MessageReportWizard({
  ...
  const [state, dispatch] = useReducer(messageReportReducer, {
    category: "spam",
    details: "",
    exactMessage: "",
    reportedMessageId: initialMessageId || "",
    screenshots: [],
    screenshotPreviews: [],
    isSubmitting: false,
    error: null
  ...
  useMemo(() => {
    if (isOpen && initialMessageId) {
      dispatch({ type: "SET_REPORTED_MESSAGE_ID", payload: initialMessageId });
    }
  ...
  const activeCategoryMeta = useMemo(() => CATEGORY_META.find((c) => c.key === state.category), [state.category]);

  const reset = () => {
    state.screenshotPreviews.forEach((preview) => {
  ...
  const removeScreenshot = (index: number) => {
    URL.revokeObjectURL(state.screenshotPreviews[index]);
  ...
  const handlePasteMessageId = async () => {
    try {
      ...
      dispatch({ type: "SET_REPORTED_MESSAGE_ID", payload: normalized });
    } catch {
  ...
  const submit = async () => {
    ...
    try {
      ...
      formData.append("exactMessage", state.exactMessage);
      formData.append("reportedMessageId", state.reportedMessageId);
          ...
          >
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar-premium p-8 space-y-8">
              ...
              <section>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {CATEGORY_META.map((c) => {
                    const Icon = c.icon;
                    const isActive = state.category === c.key;
                    ...
                    const colorMap: Record<string, { icon: string; bg: string; border: string; ring: string }> = {
                      ...
                      slate: { icon: "text-zinc-500", bg: "bg-zinc-500/10", border: "border-zinc-500/30", ring: "ring-zinc-500/5" },
                    };
              ...
              <section>
                <div className="flex items-center justify-between mb-3 ml-1">
                  ...
                  <span className="text-[10px] font-black uppercase tracking-widest text-disabled">{state.screenshots.length}/4</span>
                </div>
              ...
              {state.error && (
                ...
                >
                  ...
                  <p className="text-xs font-black text-red-500">{state.error}</p>
                </motion.div>
              )}
            </div>
            ...
            <div className="shrink-0 border-t border-divider p-6 bg-surface-1/50 flex items-center justify-between gap-4">
              <div className="hidden sm:flex items-center gap-2 text-disabled">
                ...
                <span className="text-[10px] font-black uppercase tracking-widest">Confidential Submission</span>
              </div>
              <div className="flex items-center gap-3 w-full sm:w-auto">
                ...
                <button
                  type="button"
                  onClick={submit}
                  disabled={state.isSubmitting || !hasAnyEvidence || !targetUserId || !conversationId}
                  className={cn(
