...
export default function AnalyticsPage() {
    ...
    const handleExportCsv = async () => {
        ...
        try {
            const response = await fetch("/api/landlord/analytics/report", {
                method: "POST",
                headers: { "Content-Type": "application/json" },
                body: JSON.stringify({
                    format: "csv",
            ...
            downloadBlob(blob, `landlord-portfolio-analytics-${datePart}.csv`);
    ...
    const handleExportPdf = () => {
        ...
        appendBlock("Portfolio Analytics Report", 18, 20);
    ...
    useEffect(() => {
        ...
        const loadOverview = async () => {
            ...
            try {
                ...
                const response = await fetch(`/api/landlord/analytics/overview?${params.toString()}`, {
                    method: "GET",
        ...
        <div className="relative min-h-screen w-full overflow-hidden bg-background selection:bg-primary/30">
                ...
                <div className="flex flex-wrap items-center gap-3">
                    <div className="group/iris-toggle relative">
                        <button
                            ...
                            className={cn(
                                "inline-flex items-center gap-2.5 rounded-full border px-5 py-3 text-[11px] font-semibold uppercase tracking-[0.14em] shadow-sm transition-all hover:scale-105 active:scale-95",
                                isIrisVisible
            ...
            <section className="relative z-0 w-full rounded-[2.5rem] border border-border bg-card p-8 shadow-xl shadow-black/[0.04] dark:shadow-black/20">
                <div className="flex flex-col gap-6 xl:flex-row xl:items-center xl:justify-between">
                    ...
                    <div className="flex flex-wrap items-center gap-3">
                        {statsLoading && (
                            <div className="relative overflow-hidden rounded-full border border-blue-500/20 bg-blue-500/10 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.15em] text-blue-400 shadow-sm">
                        ...
                        {statsError && (
                            <div className="rounded-full border border-red-500/20 bg-red-500/10 px-3 py-1.5 text-[10px] font-semibold uppercase tracking-[0.15em] text-red-400">
                                {statsError}
                            </div>
                        )}
                        <button
                            ...
                            className={cn(
                                "flex items-center gap-2.5 rounded-full border px-5 py-2.5 text-[10px] font-semibold uppercase tracking-[0.15em] transition-all hover:scale-105 active:scale-95 shadow-sm",
                                showMoreKpis
            ...
            <section className="relative z-0 w-full rounded-[2.5rem] border border-border bg-card p-8 shadow-xl shadow-black/[0.04] dark:shadow-black/20">
                <div className="mb-10 flex flex-wrap items-center justify-between gap-4 px-2">
                    <div className="flex min-w-0 items-center gap-4">
                        ...
                        <div>
                            <h2 className="text-2xl font-semibold tracking-tight text-foreground">Financial Performance</h2>
            ...
            <section className="relative z-0 w-full rounded-[2.5rem] border border-border bg-card p-8 shadow-xl shadow-black/[0.04] dark:shadow-black/20">
                <div className="mb-6 flex items-center gap-4 px-2">
                    ...
                    <div>
                        <h2 className="text-2xl font-semibold tracking-tight text-foreground">Export History</h2>
                ...
                {exportHistory.length === 0 ? (
                    <div className="flex flex-col items-center justify-center rounded-[1.75rem] border border-border bg-muted/20 py-12 text-muted-foreground">
                        ...
                        <p className="text-[10px] font-semibold uppercase tracking-widest opacity-40">No reports exported yet</p>
                    </div>
                ) : (
                            ...
                            return (
                                ...
                                >
                                    <div className="flex items-center justify-between">
                                        <span className={cn(
                                            "font-semibold text-[9px] tracking-[0.2em] uppercase px-2.5 py-1 rounded-full border",
                                    ...
                                    {!isExpired && (
                                        <button
                                            ...
                                            className="mt-2 flex w-full items-center justify-center gap-2 rounded-xl bg-card py-2 text-[10px] font-semibold uppercase tracking-widest text-primary border border-primary/20 transition-all hover:bg-primary hover:text-primary-foreground"
                                        >
                ...
                {hasMoreHistory && exportHistory.length > 0 && (
                    <div className="mt-8 flex justify-center">
                        <button
                            ...
                            className="group flex items-center gap-2 rounded-full border border-border bg-card px-8 py-3 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground transition-all hover:border-primary/30 hover:bg-primary/5 hover:text-primary active:scale-95"
                        >
                            Load Older Reports
                ...
                <div className="fixed inset-0 z-[120] flex items-center justify-center p-4">
                    ...
                    <div className="relative z-10 w-full max-w-lg overflow-hidden rounded-[2.5rem] border border-border bg-card shadow-2xl animate-in zoom-in-95 duration-300">
                        ...
                        <div className="p-8 flex flex-col gap-8">
                            ...
                            <div className="flex flex-col gap-4">
                                <label className="flex items-center gap-3 text-sm font-semibold text-foreground">
                                    <div className="flex size-7 items-center justify-center rounded-full border border-primary/20 bg-primary/10 text-[10px] font-semibold text-primary">1</div>
                                ...
                                <div className="pl-10 flex flex-col gap-4">
                                    <div className="flex flex-wrap items-center gap-2">
                                        {RANGE_OPTIONS.map((option) => (
                                            <button
                                                ...
                                                className={cn(
                                                    "px-4 py-2.5 rounded-2xl text-xs font-semibold uppercase tracking-[0.1em] transition-all duration-200 border",
                                    ...
                                    <div className="flex items-center gap-3">
                                        <div className="flex-1 flex flex-col gap-2">
                                            <span className="text-[10px] font-semibold uppercase tracking-[0.2em] text-muted-foreground/60">Start</span>
                                            <input
                                                type="date"
                                                value={startDate}
                                        ...
                                        <div className="pt-7 text-muted-foreground/40 font-semibold">–</div>
                                        <div className="flex-1 flex flex-col gap-2">
                                            <span className="text-[10px] font-semibold uppercase tracking-[0.2em] text-muted-foreground/60">End</span>
                                            <input
                                                type="date"
                                                value={endDate}
                            ...
                            <div className="flex flex-col gap-4">
                                <label className="flex items-center gap-3 text-sm font-semibold text-foreground">
                                    <div className="flex size-7 items-center justify-center rounded-full border border-primary/20 bg-primary/10 text-[10px] font-semibold text-primary">2</div>
                                    Select Format
                                </label>
                                <div className="pl-10 grid grid-cols-2 gap-4">
                                    ...
                                    >
                                        ...
                                        <span className="text-xs font-semibold uppercase tracking-[0.15em]">PDF</span>
                                    </button>
                                    <button
                                    ...
                                    >
                                        ...
                                        <span className="text-xs font-semibold uppercase tracking-[0.15em]">CSV</span>
                                    </button>
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex justify-end gap-3 border-t border-white/10 bg-card/95 px-8 py-6 backdrop-blur-xl">
                            <button
                                onClick={() => setIsExportModalOpen(false)}
                                className="rounded-2xl border border-white/10 bg-card/70 px-5 py-3 text-[10px] font-semibold uppercase tracking-widest text-muted-foreground/70 transition-all hover:bg-card hover:text-foreground"
                            >
                                Cancel
                            </button>
                            <button
                                ...
                                className="group rounded-2xl bg-primary px-6 py-3 text-sm font-semibold uppercase tracking-tight text-primary-foreground shadow-lg shadow-primary/20 transition-all hover:scale-[1.02] hover:bg-primary/90 active:scale-95"
                            >
                                Generate & Download
                            </button>
                        </div>
            ...
            {toastMessage && (
                <div className="fixed bottom-6 right-6 z-50 rounded-2xl border border-emerald-500/25 bg-emerald-500/12 px-5 py-3.5 text-sm font-semibold text-emerald-300 shadow-[0_20px_40px_rgba(0,0,0,0.4)] backdrop-blur-xl animate-in slide-in-from-bottom-4 duration-300">
                    {toastMessage}
                </div>
            )}
            </div>
