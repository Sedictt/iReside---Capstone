...
export async function POST(
  ...
  if (appError) {
    console.error("[regenerate-signing-link] Application fetch error:", appError);
    return NextResponse.json(
  ...
  if (lease.status !== "pending_signature" && lease.status !== "pending_tenant_signature") {
    return NextResponse.json(
      { error: `Cannot regenerate signing link for lease with status: ${lease.status}` },
      { status: 409 }
    );
  }
  ...
  // Correct arguments for generateSigningToken: leaseId, tenantId
