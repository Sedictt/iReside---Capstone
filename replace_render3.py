...
new_gsap = """    useGSAP(() => {
        ...
        mm.add("(min-width: 1024px) and (prefers-reduced-motion: no-preference)", () => {
            ...
            const irisInner = irisSection.querySelector(".iris-inner-container");
            const irisHeader = irisSection.querySelectorAll(".iris-header-anim");
            const irisCards = gsap.utils.toArray(".iris-card");
            ...
            const outcomeItems = gsap.utils.toArray(".outcome-item");
            ...
            const ctaReveal = gsap.utils.toArray(".cta-reveal");
            const ctaPanelWrapper = ctaSection.querySelector(".cta-panel-wrapper");
            const ctaAmbient = ctaSection.querySelector(".cta-ambient");

            gsap.set(ctaReveal, { autoAlpha: 0, y: 60, scale: 0.95 });
            if (ctaPanelWrapper) gsap.set(ctaPanelWrapper, { autoAlpha: 0, y: 200, scale: 0.8, rotateX: 20 });
        ...
        mm.add("(max-width: 1023px) and (prefers-reduced-motion: no-preference)", () => {
            const irisHeaders = gsap.utils.toArray(".iris-header-anim");
            const irisCards = gsap.utils.toArray(".iris-card");
            const outcomeItems = gsap.utils.toArray(".outcome-item");
            const ctaElems = gsap.utils.toArray(".cta-reveal, .cta-panel-wrapper");

            gsap.fromTo(irisHeaders, { autoAlpha: 0, y: 40 }, {
                autoAlpha: 1, y: 0, duration: 0.8, stagger: 0.15,
                scrollTrigger: { trigger: irisSection, start: "top 80%" },
            });

            gsap.fromTo(irisCards, { autoAlpha: 0, y: 60 }, {
                autoAlpha: 1, y: 0, duration: 0.8, stagger: 0.15,
                scrollTrigger: { trigger: irisSection, start: "top 75%" },
            });

            gsap.fromTo(outcomeItems, { autoAlpha: 0, y: 60 }, {
                autoAlpha: 1, y: 0, duration: 0.8, stagger: 0.15,
                scrollTrigger: { trigger: outcomesSection, start: "top 75%" },
            });

            gsap.fromTo(ctaElems, { autoAlpha: 0, y: 50 }, {
                autoAlpha: 1, y: 0, duration: 0.8, stagger: 0.15,
...
if old_gsap_start != -1 and old_gsap_end != -1:
    text = text[:old_gsap_start] + new_gsap + text[old_gsap_end + len('        });\n    });\n\n    return ('):]
else:
...
new_jsx = """            {/* iRis Section - Enhanced Scrollytelling */}
            <section
                ref={irisSectionRef}
                className="relative min-h-[140vh] py-24 md:py-32 px-6 border-t border-border overflow-hidden z-20 bg-background perspective-1000"
                ...
                <div className="iris-inner-container relative max-w-7xl mx-auto flex flex-col items-center text-center">
                    ...
                    <div className="mt-16 md:mt-24 grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 md:gap-8 w-full relative z-10" style={{ transformStyle: "preserve-3d" }}>
                        {IRIS_WORKFLOWS.map((item, idx) => (
                            <article key={item.title} className="iris-card rounded-[2rem] border border-primary/20 bg-background/60 backdrop-blur-xl px-6 py-8 shadow-2xl relative overflow-hidden group hover:border-primary/50 transition-colors h-[18rem] flex flex-col justify-center">
                    ...
                    <div className="lg:w-1/2 grid grid-cols-1 gap-y-12 md:gap-y-16">
                        {OUTCOMES.map((outcome) => (
                            <article key={outcome.label} className="outcome-item relative pl-8 border-l-[3px] border-primary/30 py-4 bg-background/30 backdrop-blur-sm rounded-r-3xl pr-6">
                                <div className="absolute -left-[10px] top-1/2 -translate-y-1/2 h-8 w-[5px] bg-primary rounded-full shadow-[0_0_15px_rgba(109,152,56,0.9)]" />
